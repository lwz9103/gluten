...
<script type="text/javascript">
          ...
          <div id="header">
            ...
            <h1>
              ...
              <strong>
                ...
                <span class="total">
                  <xsl:text>, pass ratio is </xsl:text>
                  <xsl:variable name="total_cnt" select="count[@name='total']/@value"/>
                  <xsl:variable name="passed_cnt" select="count[@name='passed']/@value"/>
                  <xsl:value-of select="$passed_cnt div $total_cnt"/>
                </span>
              </strong>
            </h1>
            <div id="treecontrol">
  ...
  <xsl:template match="suite">
    ...
    <xsl:element name="li">
      ...
      <span>
        ...
        <xsl:value-of select="@name"/>
        <xsl:variable name="total_cnt" select="count(test)"/>
        <xsl:variable name="passed_cnt" select="count(test[@status='passed'])"/>
        <em class="time">
          <xsl:value-of select="concat($passed_cnt, ' | ', $total_cnt)"/>
        </em>
      </span>
      <ul>
        <xsl:apply-templates/>
  ...
  <xsl:template match="test">
    ...
    <li class="{$class}">
      ...
      <xsl:if test="count(./output) > 0 and @status != 'passed'">
        <ul>
