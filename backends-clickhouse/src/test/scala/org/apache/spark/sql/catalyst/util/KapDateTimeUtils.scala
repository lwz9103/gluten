...
object KapDateTimeUtils {
  ...
  /**
   * Finds the number of months between two dates, each represented as the number of days since the
   * epoch.
   */
  ...
  def dateAddMonths(date: Int, m: Int): Int = {
    // dirty hack because of the lack of org.apache.calcite.avatica.util.DateTimeUtils
    var y0 = DateTimeUtils.getYear(date)
    var m0 = DateTimeUtils.getMonth(date)
    var d0 = DateTimeUtils.getDayOfMonth(date)
    ...
    LocalDate.of(y0, m0, d0).toEpochDay.toInt
  }
